...
```{r global, include=FALSE}
  ...
  df <- read_feather(here("data_for_app", "df.feather"))#employment data
  old_forecast <- read_feather(here("data_for_app", "old_forecast.feather"))%>%
    rename(employment = forecast)
  driver_data <- read_feather(here("data_for_app", "driver_data.feather"))#need to fix
  constraint <- read_csv(here("raw_data", "constraint.csv"))
  #variables----
  end_of_horizon <- year(today()) + 11#the last period to forecast
  forecast_years <- max(df$year)+1:11 #for the slider inputs
  first_year_of_forecast <- max(df$year)+1 #forecast cagr and plot annotations relative to this date.
  series <- unique(df$industry)#the industries
  industries_with_driver_data <- unique(driver_data$industry) #driver data is available for limited set of industries.
  
  #preliminary calculations----
  industry_share_last_year <- df%>%
    filter(year == max(year), industry != "ind00-ALL")%>%
    summarize(industry_proportion = round(100*employment/sum(employment), 2),
              industry = industry)
  current_forecasts <- read_csv(here("forecast_output", "forecasts.csv"))%>%#reads in the forecasts made thus far...
...
selectInput("train_vs_test", label = "First year of test set",
            choices = (max(df$year)-5):(max(df$year)+1) , selected = max(df$year)-5)
selectInput("chosen", 
...
renderPlot({
  ...
  #read in the forecast made already for feedback regarding budget constraint----
  if(file.exists(here("forecast_output", "forecasts.csv"))){# if we have made some forecasts this file exists
    ...
    forecasted_industries <- forecasts%>%
      select(industry)%>%
      distinct()
    `Percent Forecasted`<- inner_join(industry_share_last_year, forecasted_industries)%>%
    ...
    forecasted_by_year<- forecasts%>%
      ...
      inner_join(constraint)%>% #the budget constraint (top line employment)
  ...
  forecasts <- fit %>%
    ...
    mutate(model_label = if_else(year == max(year), 
                           as.character(.model), 
                           NA_character_))
  ...
  if(input$train_vs_test < max(filtered_df$year)){
    rmse <- accuracy(forecasts, filter(filtered_df, train_vs_test == "Testing"))%>%
      select(.model, RMSE)%>%
  ...
  #bind last year's forecasts and driver data to the new forecasts----
  forecasts <- forecasts%>%
    as_tibble()%>%
    select(-employment)%>%
    rbind(old_forecast)%>%
  ...
  old_cagr <- bind_rows(cagr_percent(filtered_df, historic_start, historic_end),
  ...
  new_cagr <- bind_rows(cagr_percent(filtered_df, historic_start, historic_end),
    ...
    rename(`New forecast CAGR` = cagr) 
  }
  
  #create plot based on filtered_df(only the selected industry)----
  p <- ggplot()+
    scale_y_continuous(labels = comma)+
    geom_table_npc(mapping = aes(npcx = c(.05, .3, .95),#add the tables
                              npcy = c(.05,.05,.05),
                              label = list(old_cagr, new_cagr, forecasted_by_year)))+
    geom_line(filtered_df, mapping=aes(year, 
                                       employment, 
                                       colour = train_vs_test),
              lwd = 2, 
              alpha = .25)+
    geom_point(filter(filtered_df,
                      year == historic_start |
                      year == historic_end | 
                      year == max(filtered_df$year)), 
               mapping=aes(year, 
                           employment, 
                           colour = train_vs_test), 
               size = 3, 
    ...
    geom_label_repel(data = tail(filtered_df, 1),#label the last observed employment level
                     mapping = aes(year,
                                 employment,
                                 colour = train_vs_test,
                                 label = label_comma(accuracy = 100)(employment)))+
    geom_label_repel(data = filter(filtered_df, 
                                  year == historic_start |
                     ...
                     mapping = aes(year,
                                 ...
                                 colour = train_vs_test,
                                 label = label_comma(accuracy = 100)(employment)))+
    geom_label_repel(data = head(filtered_df, 1),#label the training data.
                     mapping = aes(year,
                                 employment,
                                 colour = train_vs_test,
                                 label = "training data"))
  ...
  #modify the y axis to include 0 if desired----
    if(input$show_zero == TRUE){#change the y axis limits to include zero
      p <- p+ 
        scale_y_continuous(labels = comma,
                           limits = c(0, 
                                      max(max(filtered_df$employment), 
                                          max(adjusted_forecasts$adjusted_forecast))))
    }
  #if testing data show best model in bold----
    if(input$train_vs_test < max(filtered_df$year)){# if there is some testing data bold best model
       p <- p+
  ...
  #modify the y axis to include 0 if desired----
    forecast_max <- adjusted_forecasts%>%
      ...
      if(input$show_zero == TRUE){#change the y axis limits to include zero
        p <- p+ 
          scale_y_continuous(labels=comma, 
                             limits = c(0,max(max(filtered_df$employment),
                                                            forecast_max, 
  ...
  #                      mapping = aes(year, `Point Forecast`, label = model_label), alpha = .25)
  # }
